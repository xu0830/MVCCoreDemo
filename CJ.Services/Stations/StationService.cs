...
namespace CJ.Services.Stations
{
    public class StationService : IStationService
    {
        ...
        public LoginServiceDto ValidateLogin(StationServiceInput input)
        {
            ...
            request_2.AddParameter("application/x-www-form-urlencoded", "username=" + input.UserName + "&password=" + input.Password + "&appid=otn" + "&answer=" + loginAnswer,
                ParameterType.RequestBody);
            ...
            //if (response.Cookies.Count > 0 && response.Cookies != null)
            //{
            //    request_2.AddParameter("RAIL_DEVICEID", response.Cookies.Where(c => c.Name == "RAIL_DEVICEID").FirstOrDefault().Value, ParameterType.Cookie);

            //    request_2.AddParameter("RAIL_EXPIRATION", response.Cookies.Where(c => c.Name == "RAIL_EXPIRATION").FirstOrDefault().Value, ParameterType.Cookie);
            //}
