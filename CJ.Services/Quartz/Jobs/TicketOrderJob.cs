            ...
            return Task.Run(() =>
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        List<TicketDto> orderTickets = new List<TicketDto>();

                        if (input.TrainCodes != null && input.TrainCodes.Count > 0)
                        {
                            foreach (var item in input.TrainCodes)
                            {
                                var orderTicket = tickets.Where(c => c.Station_train_code == item).FirstOrDefault();
                                if (orderTicket != null)
                                {
                        ...
                        if (orderTickets.Count > 0)
                        {
                            foreach (var orderTicket in orderTickets)
                            {
                                if (orderTicket.CanWebBuy == "Y")
                                {
                                    ...
                                    CommonResponse<ConfirmSingleResponseData> confirmSingleForQueueResponse = null;
                                    try
                                    {
                                        confirmSingleForQueueResponse = JsonConvert.DeserializeObject<CommonResponse<ConfirmSingleResponseData>>(response_6.Content);
                                    }
                                    catch (Exception)
                                    {
